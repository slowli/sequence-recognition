...
public final class EMTasks {
	...
	public static void mixtureCV(String[] args) throws IOException {
		...
		SeqAlgorithm[] algs = new SeqAlgorithm[mixture.size()];
		for (int i = 0; i < mixture.size(); i++) {
			algs[i] = new GeneViterbiAlgorithm(6, false, set);
		}
		final SeqAlgorithm swAlgorithm = new MixtureSwitchAlgorithm(mixture, set, algs);
		
		algorithm.train(set);
		swAlgorithm.train(set);
		
		SequenceSet controlSet = set.filter(new SequenceSet.Filter() {
			@Override
			public boolean pass(int index, byte[] observed, byte[] hidden) {
				return (index < 200);
			}
		});
		...
		for (int i = 0; i < controlSet.length(); i++) {
			System.out.println("\nSequence #" + i);
			byte[] observed = controlSet.observed(i);
			byte[] hidden = algorithm.run(observed);
			double mixP = mixture.estimate(observed, hidden);
			System.out.println("mixture: logP = " + mixP);
			
			hidden = swAlgorithm.run(observed);
			double swP = mixture.estimate(observed, hidden);
			System.out.println("switch: logP = " + swP);
			
			if (swP > mixP) {
				double[] chainP = new double[mixture.size()];
				for (int k = 0; k < mixture.size(); k++) {
					chainP[k] = mixture.chains[k].estimate(observed, hidden) 
							+ Math.log(mixture.weights[k]);
				}
				System.out.println("chainP: " + Arrays.toString(chainP));
			}
		}
	 ...
	 *    в случае ошибки ввода/вывода
	 */
	@Task(id="em.features",res=Messages.BUNDLE_NAME)
	 ...
	 *    в случае ошибки ввода/вывода
	 */
	@Task(id="em.classes",res=Messages.BUNDLE_NAME)
	public static void classes(String[] args) throws IOException {
		...
		Env.debug(1, Messages.format("misc.dataset", set.repr()));
		Env.debug(1, Messages.format("em.mixture", mixture.repr()));
		Env.debug(1, "");
		
		final PrintStream outStream = new PrintStream(outFile);
		Env.debug(1, Messages.format("task.em.classes.out", outFile));
	 ...
	 *    в случае ошибки ввода/вывода
	 */
	@Task(id="em.likelihood",res=Messages.BUNDLE_NAME)
	public static void likelihood(String[] args) throws IOException {
		...
		for (int i = 1; i < args.length; i++) {
			...
			System.out.println(Messages.format("task.em.likelihood.p", logProb));
		}
	}
}
